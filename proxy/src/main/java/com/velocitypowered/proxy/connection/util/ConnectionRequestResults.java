...
public class ConnectionRequestResults {
  ...
  public static Impl forDisconnect(DisconnectPacket disconnectPacket, RegisteredServer server) {
    return forDisconnect(disconnectPacket.getReason().getComponent(), server);
  }

  public static Impl forUnsafeDisconnect(DisconnectPacket disconnectPacket, RegisteredServer server) {
    return new Impl(Status.SERVER_DISCONNECTED, disconnectPacket.getReason().getComponent(), server,
        false);
  }
